...
## UX Rule
Every user should always know where they are, what they can do next, and whether a feature is live or preview-only.

## Navigation
Primary nav must include Discover, Cities, Categories, Events, Creators, About, Sign In, Join as Artist, plus a quick search.

## Buttons
Primary actions use gradient or strong accent. Secondary actions use glass card style. Disabled or preview actions must still open a preview modal/page explaining status.

## Mobile
Mobile must be first-class. Menu must be clean, touch targets 44px minimum, no overlapping hero copy.
