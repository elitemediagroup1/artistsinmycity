...
## Secrets
No secret keys in client code. Use Netlify environment variables.

## Auth
Clerk roles: artist, fan, creator, admin.

## Permissions
Artists can edit only their own profile. Fans can manage only their own favorites/messages. Admin has moderation access.

## Payments
Use Stripe Checkout/Portal. Never store raw card data.
